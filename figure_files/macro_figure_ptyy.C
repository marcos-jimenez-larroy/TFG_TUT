...

{
  ...
  histo.str("");   histo << "/home/mjlarroy/TFG_TUT/Root_Results/mc23a_signal_histos.root"; // nombre del root de los MC
  ...
  histmc_all->Rebin(16,"hist_rebinned",new_xbins);
  /*TH1D* hist_rebinned;
  ...
  for(int j=ni; j<=nb; j++){
    ...
    anchura = (hist_rebinned->GetXaxis()->GetBinWidth(j)); //6.0 xq es la anchura más pequeña

    value = hist_rebinned->GetBinContent(j); //Valor en el bin j
    error = sqrt(value);

    hist_rebinned->SetBinContent(j,value/(anchura*L));
    ...
    hist_rebinned->SetBinError(j,error/(anchura*L));
  ...
  gPad->RedrawAxis();
  canvasp->Print("/home/mjlarroy/TFG_TUT/figures/pdf_graphs/MC_ptyy_cross_section.pdf");

  TFile* myfile = new TFile("/home/mjlarroy/TFG_TUT/Root_Results/MC_ptyy_cross_section.root", "RECREATE"); //Recreate significa que si no está lo cree y si está que lo reescriba.
