...
for(int i=1;i<=nb;i++){
  ...
  myMarkerText(x1,yy1,1,20,histo.str().c_str(),1.0,0.03,0.03);
  myLine(x1,y2,0.05,2,20,"Signal + Background",0.03,0.03);
  myLine(x1,y3,0.05,4,7,"Background",0.03,0.03);
  myLine(x1,y4,0.05,6,1000,"Signal",0.03,0.03);
  /*
  double y5 = y4 - 0.04;
  double y6 = y5 - 0.04;
  double y7 = y6 - 0.04;*/
  ...
  l.SetTextFont(72);
  l.SetTextColor(1);
  l.DrawLatex(x1+0.32,y2,"ATLAS");
  l.SetTextFont(42);

  l.DrawLatex(x1+0.32,y4,"#sqrt{s} = 13,6 TeV");
  l.DrawLatex(x1+0.32,y4-0.08,"L = 58.98 fb^{-1}");
  ...
  //double mx = 0.29;
  double mx = 0.09;
  TH1F *h2 = canvas_2->DrawFrame(bl,-200,bu,700);
  ...
  gPad->RedrawAxis();
  //
  canvasp->Print("/home/mjlarroy/TFG_TUT/figures/pdf_graphs/InclusiveRegion_22_23.pdf");
