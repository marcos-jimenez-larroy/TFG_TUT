...

{
    ...
    //El de NLO:
    TFile *ggH_NLO;  // root de todos los MC del que voy a leer
        histo.str("");   histo << "/home/mjlarroy/TFG_TUT/Root_Results/ggH.root"; // nombre del root de los MC
        ggH_NLO = new TFile(histo.str().c_str(),"read"); // abrir fichero MC
        TH1D *hist_ggH_NLO; // histograma de todos los MC que voy a mostrar


        hist_ggH_NLO = (TH1D*)ggH_NLO->Get("h_pT_truth_var_0"); // leer histograma MC

        hist_ggH_NLO->Add(hist_VBF);
        hist_ggH_NLO->Add(hist_ttH);
        hist_ggH_NLO->Add(hist_WpH);
        hist_ggH_NLO->Add(hist_WmH);
        hist_ggH_NLO->Add(hist_ZH);
    ...
    for(int j=1;j<=numBinspT;j++){
        ...
        value = hist_ggH_NLO->GetBinContent(j);
        anchura = hist_ggH_NLO->GetBinWidth(j);
        hist_ggH_NLO->SetBinContent(j,value/(anchura*L));
