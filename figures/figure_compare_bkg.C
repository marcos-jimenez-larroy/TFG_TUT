...

{
    ...
    histo.str("");   histo << "/home/mjlarroy/TFG_TUT/mc23a_bkg_histos.root"; // nombre del root del MC
    ...
    //Originally bins were of 0.6875 GeV

    TFile *data;  // root de datos que voy a leer
    TH1D *histdata; // histograma de datos que voy a leer
    histo.str("");   histo << "/home/mjlarroy/TFG_TUT/data22_histos.root"; // nombre del root de los datos
    ...
    for(int j=ni; j<=nb; j++){
      ...
      value_mc = histmc_bkg->GetBinContent(j); //Valor del MC en el bin j 
      value_data = histdata->GetBinContent(j); //Valor del dato en el bin j
      value_fit = ajuste->Eval(105+j*(160.0-105.5)/80.0); //El histograma de 105 a 160 en 80 bines.
    ...
    for(int j=ni;j<=nb;j++)
      {
        double xmc = histmc_bkg->GetBinContent(j); // signal del MC
        ...
        value_fit = ajuste->Eval(105+j*(160.0-105.5)/80.0);
    ...
    myBoxText(x1,yy1,0.05,0,132,1000,"Data",0.03,0.03);
    myBoxText(x1,y2,0.05,0,2,1000,"MC #gamma#gamma Background",0.03,0.03);
    myBoxText(x1,y3,0.05,0,3,1000,"MC reweighted Background",0.03,0.03);
    ...
    //double mx = 0.29;
    double mx = 0.09;
    TH1F *h2 = canvas_2->DrawFrame(bl,0.8,bu,1.2);
    ...
    myhhdate(histratio_ajuste,"esamex0",1.0,5,20,2.,1,1,1,5); //1.0,1,20,2.,1,1,1,1
    myhhdate(histratio,"esamex0",1.0,1,20,2.,1,1,1,1); //1.0,1,20,2.,1,1,1,1
    l.DrawLine(bl,0.2,bu,0.2);


    myLine(x1+0.4,y2-0.1,0.05,5,1000,"Fit/MC #gamma#gamma",0.08,0.03);
    myLine(x1+0.4,y2-0.18,0.05,1,1000,"Data/MC #gamma#gamma",0.08,0.03);
  
    
  
    gPad->RedrawAxis();

    histo.str("");   histo << "/home/mjlarroy/TFG_TUT/figures/pdf_graphs/MC_background.pdf";
    myepsfile(canvasp,histo.str().c_str(),"pdf");

    TFile* myfile = new TFile("/home/mjlarroy/TFG_TUT/Root_Results/MC_reweighted_background_histos.root","RECREATE");
